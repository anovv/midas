...
// TODO handle not 3:3 symbol cases

func NewCurrencyPair(pairSymbol string) CurrencyPair {
	symbolA := ""
	symbolB := ""
	letters := strings.SplitAfter(pairSymbol, "")
	for i := range(letters) {
		letter := letters[i]
		if i < 3 {
			symbolA += letter
	...
	currencyA := NewCurrency(symbolA, "")
	currencyB := NewCurrency(symbolB, "")
